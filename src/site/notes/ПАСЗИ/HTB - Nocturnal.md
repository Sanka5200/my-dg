...
ISPConfig это панель управления сервером для Linux. Звучит как серьёзная цель, к которой стоит подобраться. Вероятно, он уже где-то запущен, проверим занятость портов:
